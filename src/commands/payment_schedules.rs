...
pub async fn list(client: &mut ZuoraClient, format: OutputFormat) -> Result<()> {
    let value: serde_json::Value = client.get_json("/v1/payment-schedules").await?;
    let items = value.get("paymentSchedules").and_then(|v| v.as_array()).cloned().unwrap_or_default();
    match format {
...
pub async fn get(client: &mut ZuoraClient, key: &str, format: OutputFormat) -> Result<()> {
    let value: serde_json::Value = client.get_json(&format!("/v1/payment-schedules/{key}")).await?;
...
pub async fn create(client: &mut ZuoraClient, body_json: &str, format: OutputFormat) -> Result<()> {
    ...
    let result: serde_json::Value = client.post_json("/v1/payment-schedules", body).await?;
    println!("{}", format_value(&result, format));
    Ok(())
}

pub async fn cancel(client: &mut ZuoraClient, key: &str, format: OutputFormat) -> Result<()> {
    let result: serde_json::Value = client.put_json(&format!("/v1/payment-schedules/{key}/cancel"), serde_json::json!({})).await?;
    println!("{}", format_value(&result, format));
    Ok(())
}

pub async fn delete(client: &mut ZuoraClient, key: &str, format: OutputFormat) -> Result<()> {
    let result = client.delete_req(&format!("/v1/payment-schedules/{key}")).await?;
    println!("{}", format_value(&result, format));
    Ok(())
}
