...
pub async fn list(client: &mut ZuoraClient, format: OutputFormat) -> Result<()> {
    let value: serde_json::Value = client.get_json("/v1/sequence-sets").await?;
    let items = value.get("sequenceSets").and_then(|v| v.as_array()).cloned().unwrap_or_default();
    match format {
...
pub async fn get(client: &mut ZuoraClient, id: &str, format: OutputFormat) -> Result<()> {
    let value: serde_json::Value = client.get_json(&format!("/v1/sequence-sets/{id}")).await?;
...
pub async fn create(client: &mut ZuoraClient, body_json: &str, format: OutputFormat) -> Result<()> {
    ...
    let result: serde_json::Value = client.post_json("/v1/sequence-sets", body).await?;
    println!("{}", format_value(&result, format));
    Ok(())
}

pub async fn delete(client: &mut ZuoraClient, id: &str, format: OutputFormat) -> Result<()> {
    let result = client.delete_req(&format!("/v1/sequence-sets/{id}")).await?;
    println!("{}", format_value(&result, format));
    Ok(())
}
